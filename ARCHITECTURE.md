...
└── src/
    ...
    │   ├── mentor-context.ts   # profile block + fact-grounded scholarship retrieval
    │   ├── scholarship-facts.ts# canonical scholarship/deadline rendering for the LLM
    │   ├── rag-ingest.ts       # chunking + embedding for ScholarshipDoc
...
chat history for anon users). Never expose the service key to the client.
...
Chunk 0 of every scholarship is its authoritative fact sheet; every other chunk
is prefixed with a `[Title — Country | Deadline: ...]` identity line. Retrieval
returns fragments from the middle of a description, and without that prefix the
model could not tell which programme (or deadline) a fragment belonged to.
Re-run `pnpm --filter web reingest` after changing the chunk format.
...
> **Removed:** `prompt_cache` (and the `bump_prompt_cache_hit` RPC). Every
> mentor answer is now personalised with the student's profile and grounded in
> live scholarship rows, so cache keys were effectively unique per request —
> lifetime hit rate was 59 hits across 143 entries. See migration `021`.
...
- Flow per request:
  ...
  4. **Profile block** (`lib/mentor-context.ts`): the signed-in student's
     profile is summarised and injected into the system prompt on every turn, so
     answers are personalised without the client sending (or forging) it.
  5. **Retrieval** (`lib/mentor-context.ts`): embed the last user message (NIM)
     → `match_scholarship_docs`, widened by a title-keyword probe. The matched
     scholarship IDs are then **re-read from the `scholarships` table** and
     rendered as fact sheets. Prose chunks ride along as background only. This
     is what keeps deadlines exact — the model is told to quote the live row,
     never the embedded text. `body.scholarshipId` pins the detail page's
     scholarship into the block regardless of the query.
  6. Stream from OpenRouter. First SSE event announces the real model
     (`{ model }`), then `{ token }` events, then `[DONE]`.
  7. Persist assistant message (logged-in only — anon chats stay client-side).
- The system prompt is **server-owned**. Any `system` message a client sends is
  folded in as supplementary "page context"; it never replaces the base prompt.
- Limits: `MAX_OUTPUT_TOKENS = 4096`, `MAX_HISTORY = 12` turns.
...
1. **Rate limits** per tier — the main control (see the table above).
2. **Output cap** 4096 tokens, **history cap** 12 turns.
3. Embeddings are free (NIM).
4. Admin parse/enrich are infrequent and selectable to a cheap model.

Response caching was tried and removed (migration `024`) — see §11.
...
chat/page.tsx → POST /api/chat
  → resolve tier + load profile (mentor-context.ts) + rate-limit (rate-limit.ts)
  → embed question (nim.ts) → match_scholarship_docs RPC + title probe
      → hydrate exact rows from `scholarships` → verified-facts block
  → system prompt = base rules + today's date + profile block + facts block
  → stream from OpenRouter (openrouter.ts) → persist assistant turn
...
- **Don't reintroduce a response cache for the mentor** without solving the two
  problems that killed the last one: answers embed the student's profile (so a
  shared entry leaks one student's advice to another) and quote live deadlines
  (so an entry goes wrong the moment an admin edits a row). Keying around both
  made hits vanishingly rare, which is why it was removed.
- **The mentor must never approximate a deadline.** `scholarships.deadline` is
  free text; render it through `lib/scholarship-facts.ts` so every surface shows
  the same value, and keep the grounding rules in the chat system prompt intact.
